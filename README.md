...
当前版本：**0.26.0**。仓库中的部署状态以 [Actions](https://github.com/1157364449han-max/-/actions) 的实际结果为准，上传源码不代表网页已经上线。

## 使用能力与边界

0.26.0 改进本地 OCR 的方向纠正、图像增强复核及置信度提示。增加云端视觉识题接口：仅在独立云服务就绪、用户主动勾选后上传题图，识别结果仍须核对。云端部署入口：[Deploy to Render](https://render.com/deploy?repo=https://github.com/1157364449han-max/-)。创建时 Gemini 密钥只填 Render Secret，绝不提交仓库；未完成部署与真实验收前，公开网页仍仅使用浏览器本地 OCR。

0.25.0 增加弦长、方向角、sin θ、cos θ 的数值最值扫描。题目询问最大、最小或范围时自动预选观察量；可跳转极值位置，画板高亮弦和交点并标注取值条件。扫描只是探索，不代替严格证明；尚不能自动识别任意题目中的自定义量。

0.24.0 新增手机拍照和电脑上传题图。图片在浏览器本地由 Tesseract.js 识别，识别文字必须经用户核对后才成为题目；公式、上下标、手写体仍可能识错。首次使用需联网下载 OCR 组件和中英文语言数据；图片不上传到云端。此功能不等于通用 AI 解题，云服务配置留待后续。
