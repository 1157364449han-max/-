module.exports=async({page,context,assert})=>{
  ...
  await context.route('**/runtime-config.js',route=>route.fulfill({contentType:'application/javascript',body:'window.DONGJIEXI_CONFIG={version:"0.21.1",deployment:"web",apiEnabled:true,apiBase:"",requiresAuth:false};'}));
  ...
  await context.route('**/runtime-config.js',route=>route.fulfill({contentType:'application/javascript',body:'window.DONGJIEXI_CONFIG={version:"0.21.1",deployment:"web",apiEnabled:false,apiBase:"",requiresAuth:false};'}));
  await page.reload({waitUntil:'domcontentloaded'});
